...
class SWTScrubber(object):
    ...
    def _create_derivative(cls, filepath):
        ...
        if diagnostics:
            cv2.imwrite('diagnostics/edges.jpg',edges)
            cv2.imwrite('diagnostics/sobelx64f.jpg', np.absolute(sobelx64f))
            cv2.imwrite('diagnostics/sobely64f.jpg', np.absolute(sobely64f))
            # amplify theta for visual inspection
            theta_visible = (theta + np.pi)*255/(2*np.pi)
            cv2.imwrite('diagnostics/theta.jpg', theta_visible)
    ...
    def _swt(self, theta, edges, sobelx64f, sobely64f,light_on_dark):
        ...
        if diagnostics:
            cv2.imwrite('diagnostics/swt.jpg', swt * 100)
    ...
    @classmethod
    def _find_letters(cls, swt, contours,size):
        # STEP: Discard shapes that are probably not letters
        boxes = []
        for label,layer in contours.iteritems():

            layer = np.array(layer)
            x,y,width,height = cv2.boundingRect(layer)
            ...
            median_swt = np.median(swt[layer[:,1], layer[:,0]])

            if diameter / median_swt > 10:
                continue

            if width / size[1] > 0.4 or height / size[0] > 0.4:
                continue
